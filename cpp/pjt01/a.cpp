...
size_t mstrnlen(const char *s, size_t maxlen) {
  size_t len = 0;
...
std::ostream &operator<<(std::ostream &os,
                         const unordered_map<K, V, str_hash, str_eq> &m) {
  size_t sz = m.size();
  os << "{ " << "\n\t" << "size : " << sz << ", ";
  os << "\n\t" << "data : [";
...
int main() {
  ...
  size_t max_cmd_len = 256;
  string cmd;
  unordered_map<const char *, Item *, str_hash, str_eq> m;
  while (is_running) {
    cout << "Command (-x/--quit(-q) to quit, --add(-a) to add, --delete(-d) to "
            "delete, --list(-l, *Default) to list) : ";
    cout.flush();
    cin >> cmd;

    if (mstrnlen(cmd.c_str(), max_cmd_len) < 1) {
      cmd += "-l";
    }
    if (!strncmp("-x", cmd.c_str(), 2) || !strncmp("-q", cmd.c_str(), 2)) {
      is_running = false;
    } else if (!strncmp("-a", cmd.c_str(), 2)) {
      cout << "Enter {no} {name} {desc} : ";
      cout.flush();
      Item *newItem = new Item();
      cin >> *newItem;
      if (auto search = m.find(newItem->getName()); search != m.end()) {
        Item *old = search->second;
        m[newItem->getName()] = newItem;
        cout << "Updated. key : " << newItem->getName() << ", val: " << *(old)
             << std::endl;
        delete old;
      } else {
        m.insert({newItem->getName(), newItem});
        cout << "Inserted. key : " << newItem->getName()
             << ", val: " << *newItem;
      }
