...
int main(int argc, char **argv)
{
	    ...
	    {
			printf("WSAStartup() failed with error %lu\n", Ret);
			return 1;
	    }
	    else
	    ...
	    while(TRUE)
	    {
			if ((Accept = WSAAccept(Listen, NULL, NULL, NULL, 0)) == (long long unsigned int)SOCKET_ERROR)
			{
	    ...
	    printf("Socket number %lu got connected...\n", Accept);
...
//				, INFINITE) == 0)
		if (GetQueuedCompletionStatus(CompletionPort
				, (unsigned long int*)&BytesTransferred
				, (unsigned long long int*)(LPDWORD)&PerHandleData
				, (LPOVERLAPPED *) &PerIoData
				, INFINITE) == 0)
		{
